...
var svgWorldMap = (function () {
  ...
  var options = {
    ...
    clickCoords: "clickCoords", // return map coords on zoom pan mode | custom code
    mapOut: "mapOut",
    mapOver: "mapOver",
    mapClick: "mapClick",
    mapCoords: "mapCoords",
          ...
          let promise2 = new Promise((resolve2) => {
            ...
            svgMap = {
              ...
              coords: function (data) {
                window["getCoords"].call(null, data);
              },
              shape: function (data) {
                window["drawShape"].call(null, data);
              },
              addPoint: function (data) {
                window["addPoint"].call(null, data);
              },
              removePoint: function (data) {
                window["removePoint"].call(null, data);
              },
              date: function (data) {
              ...
              registerOnClickMapCoords: function (data) {
                window["registerOnClickMapCoords"].call(null, data);
              },
            };
            // Load time controls
  ...
  function initMap() {
    if (document.getElementById(options["mapContainer"]) == null) {
      initDefaultMap();
    } else {
      if (document.getElementById("svg-world-map-container") == null) {
        var container = document.createElement("div");
        container.setAttribute("id", "svg-world-map-container");
        let mapContainer = document.getElementById(options["mapContainer"]);
        mapContainer.prepend(container);
  ...
  function initMapCountries() {
    ...
    baseNode.childNodes.forEach(function (node) {
      ...
      if (
        ...
        (node.tagName === "g" || node.tagName === "path" || node.tagName === "rect")
      ) {
    ...
    // Add map background image, if set
    if (options.backgroundImage !== "") {
      ...
      background.setAttribute("height", "507.224");
    ...
    if (options.noBackground) {
      var world = baseNode.getElementById("World");
      world.style.fill = "rgb(255 255 255 / 0%)";
    } else {
      var world = baseNode.getElementById("World");
      world.style.fill = "rgb(255 255 255)";
    }
  ...
  function pathSetAttributes(path, event) {
    if (path != undefined && path.id != "World" && path.id != "Ocean") {
      ...
      if (
        event == "click" ||
        ((event == "out" || event == "over") && path != selectedCountry && path.country != selectedCountry)
      ) {
      ...
      } else if (typeof event === "string" && (event.substr(0, 1) === "#" || event.substr(0, 3) == "rgb")) {
  ...
  function updateInfoBox(event, path) {
    ...
    if (infoBox !== undefined) {
      if (event === "over" && path && path?.id !== "World" && path?.id !== "Ocean") {
  ...
  function registerClickMapCoords() {
    if (svg) {
      var svgViewPort = svg.getSVGDocument().children[0].querySelector("g.svg-pan-zoom_viewport");
      svgViewPort.addEventListener("dblclick", function (event) {
        const domPoint = new DOMPointReadOnly(event.clientX, event.clientY);
        const pt = domPoint.matrixTransform(svgViewPort.getScreenCTM().inverse());
        callBack("clickCoords", pt);
        //myWorldMap.shape(`<circle cx="${pt.x}" cy="${pt.y}" r="1" style="fill: red" />`);
      });

      console.log("registerClickMapCoords");
    }
  }
  ...
  window.drawShape = function (svgString) {
    ...
    //   var dataId = $(this).data("id"); //jquery
    //   var dataEvent = $(this).data("event");
    //   window[dataEvent].apply(window, ["mouseout", dataId]);
    // });
    // child.addEventListener("mouseover", function () {
    //   var dataId = $(this).data("id"); //jquery
    //   var dataEvent = $(this).data("event");
    //   window[dataEvent].apply(window, ["mouseover", dataId]);
    // });
    // child.addEventListener("click", function () {
    //   var dataId = $(this).data("id"); //jquery
    //   var dataEvent = $(this).data("event");
    //   window[dataEvent].apply(window, ["click", dataId]);
    // });

    shapes.appendChild(child);
  };

  window.addPoint = function ({ id, mapX, mapY, svg }) {
    var template = document.createElementNS(svgNS, "svg");
  ...
  window.removePoint = function (pointId) {
    ...
    if (point) {
      var pointElement = svg
        .getSVGDocument()
        .children[0].querySelector("g.svg-pan-zoom_viewport")
        .querySelector("g#shapes")
        .querySelector(`#${pointId}`);
  ...
  window.highLightCountry = function (id) {
    window.resetAllSelected();
    var country = countries[id];
    if (country) {
      fillSelected(id);
      allSelected.push(country);
    }
  };

  window.highLightCountries = function (ids) {
    window.resetAllSelected();
    if (ids) {
  ...
  window.resetAllSelected = function () {
    if (allSelected) {
      for (var i = 0; i < allSelected.length; i++) {
        var selectedOld = allSelected[i];
        resetOldSelected(selectedOld);
      }
    }
  };

  window.registerOnClickMapCoords = function () {
    registerClickMapCoords();
  };

  function fillSelected(id) {
    var country = countries[id];
    if (country !== undefined) {
      country.provinces.forEach(function (province) {
        pathSetAttributes(province, "click");
  ...
  function callBack(event, data) {
    // 'data' is a path except for coords and time controls date
    if (event === "over" && window[options.mapOver] && typeof window[options.mapOver] === "function") {
      ...
      event === "clickCoords" &&
      window[options.clickCoords] &&
      typeof window[options.clickCoords] === "function"
    ) {
      window[options.clickCoords].apply(window, [data]);
    }
  }
  ...
  var countryData = {
    ...
    CD: { name: "Congo (Dem. Rep.)", altnames: "Democratic Republic of the Congo,DR Congo", region: "AF" },
    ...
    EH: { name: "Western Sahara", altnames: "Sahrawi Arab Democratic Republic", region: "AF" },
    ...
    ST: { name: "Sao Tome and Principe", altnames: "São Tomé and Príncipe", region: "AF" },
  ...
  function svgWorldMapTimeControls(timePause, timeLoop, initTimeData) {
    ...
    document
      ...
      .insertAdjacentHTML("beforeend", '<link rel="stylesheet" href="' + options.libPath + 'font/flaticon.css" />');
  ...
  function initControls() {
    ...
    if (document.getElementById("map-controls") == null) {
      ...
      // Control elements
      var controlElements = {
        "map-controls": { tag: "div", append: "svg-world-map-container" },
